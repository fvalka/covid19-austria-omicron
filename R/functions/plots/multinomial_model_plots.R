...
plot_multinomial_fit <- function(data, 
  ...
  complete_plot <- samples %>%
    ...
    tidybayes::stat_lineribbon(aes(y=prop, color=variant_full_name), .width = c(0.3, 0.6, 0.9), alpha=0.25) +
...
plot_multinomial_growth_advantage <- function(fit) {
  plot(bayestestR::hdi(as.data.frame(fit$fit)$b_muomicron_days, c(0.3, 0.6, 0.9))) +
...
plot_multinomial_weekly_fold_increase <- function(fit) {
  plot(bayestestR::hdi(exp(as.data.frame(fit$fit)$b_muomicron_days*7), c(0.3, 0.6, 0.9))) +
    scale_fill_brewer(palette = "Blues", direction = -1) +
    ggpubr::theme_pubr() +
    labs(fill="CI",
...
#' Plot of more than 50% Omicron cases
#' 
...
#' Draws samples from a multinomial fit object
#' 
#' @param data Preprocessed data
#' @param fit BRMS Multinomial fit
#' @param ndraws Number of draws to draw
#' 
#' @return Dataframe with all draws 
